...
import { activateUser, deleteUser, getProfile, getUsers, login, register, resendOtp, updateProfile, updateUser, verifyOtp } from './api';
import './styles.css';
...
function App() {
  ...
  if (token && profile) {
    return <Dashboard profile={profile} users={users} pagination={pagination} userQuery={userQuery} setUserQuery={setUserQuery} onActivateUser={handleActivateUser} section={section} setSection={setSection} sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} form={form} loading={loading} notice={notice} setForm={setForm} onSave={handleProfileSave} onLogout={handleLogout} />;
  }
...
function Dashboard({ profile, users, pagination, userQuery, setUserQuery, onActivateUser, section, setSection, sidebarOpen, setSidebarOpen, form, loading, notice, setForm, onSave, onLogout }) {
  ...
  return <main className="dashboard-shell">
    ...
    <section className="dashboard-content"><div className="welcome-row"><div><span className="eyebrow">{profile.role.toUpperCase()} WORKSPACE</span><h1>Welcome, <em>{firstName}</em> <span className="wave">✦</span></h1></div><div className="role-pill"><ShieldCheck size={15} /> {profile.role}</div></div>
      {profile.role === 'admin' && <AdminDashboard users={users} pagination={pagination} query={userQuery} setQuery={setUserQuery} onActivateUser={onActivateUser} section={section} />}
...
function AdminDashboard({ users, pagination, query, setQuery, onActivateUser, section }) {
...
function UserList({ users, onActivate = async (userId) => { await activateUser(localStorage.getItem('loarn_token'), userId); window.location.reload(); }, onEdit = async (user) => { const name = window.prompt('Update user name', user.name); if (name?.trim()) { await updateUser(localStorage.getItem('loarn_token'), user._id, { name: name.trim() }); window.location.reload(); } }, onDelete = async (userId) => { if (window.confirm('Delete this user account?')) { await deleteUser(localStorage.getItem('loarn_token'), userId); window.location.reload(); } }) { return <div className="user-list">{users.map((user) => <div className="user-row" key={user._id}><div className="avatar">{user.name?.charAt(0).toUpperCase()}</div><div><strong>{user.name}</strong><small>{user.email}</small></div><span className={`status ${user.isVerified ? 'verified' : ''}`}>{user.isVerified ? 'Verified' : 'Pending'}</span><button className="manage-button" onClick={() => onEdit(user)}>Edit</button>{!user.isVerified && <button className="activate-button" onClick={() => onActivate(user._id)}>Activate</button>}<button className="delete-button" onClick={() => onDelete(user._id)}>Delete</button><ChevronRight size={16} /></div>)}{!users.length && <p className="empty-state">No users to show yet.</p>}</div>; }
